...
wompi as (
select * ,SPLIT_PART(REFERENCIA,'_',2) as wompi_reference_id
,case WHEN (AMOUNT<=40000) then (AMOUNT*0.015) else 600 end AS fee_Wompi
...
left join wompi 
on 
wompi.wompi_reference_id = base.reference_id
and wompi.amount = base.total
where 1=1 and 1=1 
)
where rn=1
group by 1
